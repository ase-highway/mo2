  ...
  $('[data-scroll-id]').on('click', function(e){
    ...
    let $this = $(this)
		...
		if($(this).parents('li').length > 0){
			...
			if($(this).parents('li').siblings('.active').length <= 0){
				$(this).parents('li').addClass('on');
			}
		}else{
...
function dateSet(){
  ...
  $(".date-setting-box .datepicker").on('change', function(){
    $(this).parents('.date-setting-box').find("input:radio").prop("checked", false);
  });

  $(".date-setting-box .btn-search").on("click", function(){
    var $box = $(this).closest('.date-setting-box');
    var startDate = $box.find('.start-date').val();
    var endDate = $box.find('.end-date').val();
    ...
    $(document).trigger('dateSearch', [{ startDate: startDate, endDate: endDate, $box: $box }]);
  });
